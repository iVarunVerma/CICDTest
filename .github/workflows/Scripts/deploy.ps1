...
$solutionRoot = "$RunnerWorkSpaceLocation\BuzzRxSmartBINClaimExport"
...
function Register-DeployClaimExtractTask {
	...
	Invoke-Command -Session $ps_session -ScriptBlock {
		...
		$taskName = "BuzzRxSmartBINClaimExport2"

		# App machine temp location
		$tempFolder = "$TempLocation\$currentDate"
		$templatePath = "$tempFolder\BuzzRxSmartBINClaimExport.xml"
		...
		if (-not $taskExists) 
		{
			...
			# Update Command
			$xml.Task.Actions.Exec.Command = "$deployLive\BuzzRxSmartBINClaimExport.exe"
...
function Deploy-ClaimExportApp {
